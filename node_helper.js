...
const handleRedirect = ({currentUrl, redirectsLeft, res, visited}) => {
  ...
  if (redirectsLeft <= 0) {
    output.error = new Error(`Too many redirects. Last: ${currentUrl}`); return output;
  }
...
const parseJsonOrError = (body) => {
  try {
    ...
    let msg = ""; if (err && err.message) {
      msg = err.message;
    } else {
      msg = String(err);
    }
    return {ok: false,
      error: new Error(`Failed to parse JSON: ${msg}`)};
  }
};
...
const handle3xxOrContinue = ({currentUrl, redirectsLeft, res, visited, requestFn, resolve, reject}) => {
  const {nextUrl, error} = handleRedirect({currentUrl,
    redirectsLeft,
    res,
    visited});
  if (error) {
    res.resume(); reject(error); return true;
  }
  if (nextUrl) {
    res.resume(); requestFn({currentUrl: nextUrl,
...
const handleResponse = async ({res, currentUrl, redirectsLeft, visited, requestFn, resolve, reject}) => {
  ...
  if (statusCode >= 300 && statusCode < 400) {
    const redirected = handle3xxOrContinue({currentUrl,
      redirectsLeft,
      res,
      visited,
      requestFn,
      resolve,
      reject}); if (redirected) {
      return;
    }
  }
  if (statusCode < 200 || statusCode >= 300) {
    await failWithStatus({res,
      statusCode,
      reject}); return;
  }
  await handle2xx({res,
...
const buildResponseHandler = (args) => (res) => handleResponse({...args,
  res});

const tryParseUrl = (currentUrl) => {
  try {
    return {ok: true,
      parsed: new URL(currentUrl)};
  } catch {
    return {ok: false,
      error: new Error(`Invalid URL: ${currentUrl}`)};
  }
};
...
const requestOnce = ({currentUrl, redirectsLeft, timeout, headers, tlsInsecure, visited, resolve, reject}) => {
  ...
  if (!parsedResult.ok) {
    reject(parsedResult.error); return;
  }
  const {parsed} = parsedResult;
  const isHttpsFlag = parsed.protocol === "https:";
  const lib = getLib(isHttpsFlag);
  const requestOptions = makeRequestOptions({parsed,
    isHttpsFlag,
    headers,
    tlsInsecure});
  const onResponse = buildResponseHandler({
    ...
    redirectsLeft,
    visited,
    requestFn: (args) => requestOnce({currentUrl: args.currentUrl,
      redirectsLeft: args.redirectsLeft,
      timeout,
      headers,
      tlsInsecure,
      visited,
...
const normalizeOptions = (payload) => {
  ...
  let timeout = 15000; if (typeof timeoutRaw === "number") {
    timeout = timeoutRaw;
  }
  const tlsInsecure = Boolean(tlsInsecureRaw);
  let maxRedirects = 3; if (typeof maxRedirectsRaw === "number") {
    maxRedirects = maxRedirectsRaw;
  }
  return {id,
    url,
    timeout,
...
module.exports = NodeHelper.create({
  ...
  async getJson (payload) {
    ...
    if (!url) {
      Log.error("[MMM-JsonTable] getJson: missing url"); this.sendSocketNotification("MMM-JsonTable_JSON_ERROR", {id,
        url,
        error: "Missing URL"}); return;
    }
    Log.log(`[MMM-JsonTable] Fetching: ${url}`);
    try {
      const data = await this.httpGetJson(url, {timeout,
        headers,
        tlsInsecure,
        maxRedirects});
      this.sendSocketNotification("MMM-JsonTable_JSON_RESULT", {id,
        url,
        data});
      ...
      this.sendSocketNotification("MMM-JsonTable_JSON_ERROR", {id,
        url,
        error: msg});
    }
  },
  ...
  httpGetJson (targetUrl, {timeout, headers, tlsInsecure, maxRedirects}) {
    return new Promise((resolve, reject) => {
      const visited = [];
      let startRedirects = 3; if (typeof maxRedirects === "number") {
        startRedirects = maxRedirects;
      }
      requestOnce({currentUrl: targetUrl,
        redirectsLeft: startRedirects,
        timeout,
        headers,
        tlsInsecure,
        visited,
